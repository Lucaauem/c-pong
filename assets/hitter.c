...
void hitter_move(Hitter* hitter, int direction) {
  hitter->object.position.y += (int)(hitter_speed * direction);

  if(hitter->object.position.y + hitter_height >= RESOLUTION_HEIGHT) {
    hitter->object.position.y = RESOLUTION_HEIGHT - hitter_height;
  } else if(hitter->object.position.y <= 0) {
    hitter->object.position.y = 0;
  }
}

void hitter_moveComputer(Hitter* hitter, Vector2 ballPosition) {
  int centerPosY = hitter->object.position.y + (hitter_height / 2);
